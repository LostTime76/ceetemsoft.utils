...
public readonly struct ResponseOptions
{
	...
	{
		// Write the file
		FileExtensions.WriteAllTextIfDifferent(filepath, ResolveArguments(arguments));
	...
	private string ResolveArguments(string? arguments)
	{
		...
		// Split the arguments
		string[] split = Sclex.Split(arguments);

		// Escape backslashes
		if (EscapeBackslashes)
		{
			EscapeArgumentBackslashes(split);
		}

		// Rejoin the arguments
		return Sclex.Join(split.AsReadOnly(),
			SplitArguments ? Environment.NewLine : Sclex.DefaultSeparator);
	}

	private static void EscapeArgumentBackslashes(string[] arguments)
	{
